...
def metrics(config: dict, path_preds: str, remove_preds=False):
    ...
    preds_msks = [
        Path(
            path_preds.as_posix(),
            i.split("/")[-1].replace(i.split("/")[-1], "PRED_" + i.split("/")[-1]),
        ).as_posix()
        for i in truth_images
    ]
    assert len(truth_msks) == len(
        preds_msks
    ), "[WARNING !] mismatch number of predictions and test files."
    print("-- Calculating metrics --")
    patch_confusion_matrices = []
    for u in range(len(truth_msks)):
        try:
            target = (
                np.array(Image.open(truth_msks[u])) - 1
            ...
            patch_confusion_matrices.append(
                confusion_matrix(
                    target.flatten(),
                    preds.flatten(),
                    labels=list(range(int(len(config["classes"])))),
                )
            )
    ...
    out_folder_metrics = Path(
        "/".join(path_preds.as_posix().split("/")[:-1]), "metrics"
    )
    ...
    for metric_name, metric_value in zip(
        metrics["Avg_metrics_name"], metrics["Avg_metrics"]
    ):
    ...
    def print_class_metrics(class_dict, metrics_available=True):
        for class_index, class_info in class_dict.items():
            ...
            if metrics_available:
                i = metrics["classes"].index(
                    class_name
                )  # Get the index of the class in the metrics
                print(
                    "{:<25} {:<15} {:<10.4f} {:<10.4f} {:<10.4f} {:<10.4f}".format(
                        class_name,
                        class_weight,
                        metrics["per_class_iou"][i],
                        metrics["per_class_fscore"][i],
    ...
    print(
        "{:<25} {:<15} {:<10} {:<10} {:<10} {:<10}".format(
            "Class", "Weight", "IoU", "F-score", "Precision", "Recall"
        )
    )
    print("-" * 65)
