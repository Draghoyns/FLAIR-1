...
def convert(img: np.ndarray, img_type: str) -> np.ndarray:
    ...
    if img_type == "class_prob":
        ...
        img_arg = np.argmax(img, axis=0)
        img_arg = np.expand_dims(img_arg, axis=0).astype(np.uint8)
        img_max = np.max(img, axis=0)
        img_max = np.expand_dims(img_max, axis=0).astype(np.float32)
...
class Sliced_Dataset(Dataset):
    ...
    def normalization(
        ...
        if norm_type == "custom":
            ...
            print(
                "Normalization argument should be 'scaling' or 'custom'. Going with scaling."
            )
            in_img = img_as_float(in_img)
